...
+(void)POST:(NSString *)URLPath
 parameters:(id)parameters
  needCache:(BOOL)needCache
    ...
    failure:(void (^)(id operation, NSError *error))failure
{
    [self requestWithUrl:URLPath method:@"POST" headers:@{@"Content-Type":@"application/json"} parameters:parameters needCache:needCache success:success failure:failure];
}
...
parameters:(id)parameters
 needCache:(BOOL)needCache
   ...
   failure:(void (^)(id operation, NSError *error))failure
{
    [self requestWithUrl:URLPath method:@"PUT" headers:@{@"Content-Type":@"application/json"} parameters:parameters needCache:needCache success:success failure:failure];
}
+(void)DELETE:(NSString *)URLPath
   parameters:(id)parameters
    needCache:(BOOL)needCache
      ...
      failure:(void (^)(id operation, NSError *error))failure
{
    [self requestWithUrl:URLPath method:@"DELETE" headers:@{@"Content-Type":@"application/json"} parameters:parameters needCache:needCache success:success failure:failure];
}
              ...
              failure:(void (^)(id operation, NSError *error))failure
{
    ...
    if ((LQAppDelegate.networkStatus == AFNetworkReachabilityStatusReachableViaWWAN) || (LQAppDelegate.networkStatus == AFNetworkReachabilityStatusReachableViaWiFi) || needCache) {
